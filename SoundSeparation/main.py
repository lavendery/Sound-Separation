...
def inference():
    ...
    sed_model = HTSAT_Swin_Transformer(
        ...
        in_chans=htsat_config.inchannel,
    ...
    model = ZeroShotASP(
        ...
        spec_size=htsat_config.htsat_spec_size,
        patch_size=htsat_config.htsat_patch_size,
        out_channels=htsat_config.inchannel,
        mel_bins=htsat_config.mel_bins,
    )
    # resume checkpoint
...
def train():
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor = "mixture_sdr",
        filename='l-{epoch:d}-{mAP:.3f}-{mAUC:.3f}-{mixture_sdr:.3f}-{clean_sdr:.3f}-{silence_sdr:.3f}',
        save_top_k = 10,
        mode = "max"
    )
    # infer at model
    ...
    # load the checkpoint
    ckpt = torch.load(htsat_config.resume_checkpoint, map_location="cpu")
    at_model.load_state_dict(ckpt["state_dict"], strict=False)
    ...
    if config.resume_checkpoint is not None:
    ...
    elif config.separation_resume_checkpoint is not None:
        ckpt = torch.load(config.separation_resume_checkpoint, map_location="cpu")
        model.load_state_dict(ckpt["state_dict"], strict = False)
    elif config.sed_resume_checkpoint is not None:
        # load pretrain sed model
        ckpt = torch.load(config.sed_resume_checkpoint, map_location="cpu")
        ...
        for key in model_params:
            pre_key = "sed_model." + key
            if pre_key in ckpt:
                assert model_params[key].shape==ckpt[pre_key].shape, "%s is not match, %s vs. %s" %(key, str(model_params[key].shape), str(ckpt[pre_key].shape))
                found_parameters.append(key)
                ckpt[key] = ckpt.pop(pre_key)
            else:
