...
class LearnOVSTFT2COMPLEX(nn.Module):
    ...
    def forward(self, x, inverse):
        if(inverse):
            ...
            x = torch.cat([x,((x[:,:,0::2,:].pow(2.0)+x[:,:,1::2,:].pow(2.0))+1e-8).log()],-2)
            # for freq_inx in range(self.nfilters):
...
# test learn ov mel
# m = LearnOVSTFT2COMPLEX(513,64,2,32,2,False,32000)
# inp = torch.rand(20,2,1001,513) # B C T F
# print('inp:',inp.shape)
# out = m(inp, False)
# print('out:',out.shape)
...
# [0, 20, 58, 128, 261, 513]
# inp: torch.Size([20, 2, 1001, 513])
# out: torch.Size([20, 32, 1001, 64])
# recover: torch.Size([20, 2, 1001, 513])