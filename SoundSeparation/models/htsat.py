...
from .ov_stft2mel_channel import LearnOVSTFT2MELChannel
from .ov_stft2complex import LearnOVSTFT2COMPLEX
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4,4), 
        ...
        if self.config.input_ov_stft2melchannel:
            self.spec_extractor_stft_abs = Spectrogram(n_fft=config.window_size, hop_length=config.hop_size, 
                win_length=config.window_size, window=window, center=center, pad_mode=pad_mode, 
                freeze_parameters=True, power=1.0)
            self.ov_stft2mel = LearnOVSTFT2MELChannel(nfreq=config.window_size // 2 + 1, nfilters=config.mel_bins, n_channels=1, dense_layer_width=self.in_chans, dense_layer_width2=1, sample_rate=config.sample_rate)
        elif self.config.input_ov_stft2complex:
            self.spec_extractor_stft = STFT(n_fft=config.window_size, hop_length=config.hop_size, 
                win_length=config.window_size, window=window, center=center, pad_mode=pad_mode, 
                freeze_parameters=True)
            self.ov_stft2complex = LearnOVSTFT2COMPLEX(nfreq=config.window_size // 2 + 1, nfilters=config.mel_bins, n_channels=2, dense_layer_width=self.in_chans, dense_layer_width2=2, sample_rate=config.sample_rate)
    ...
    def forward_features_asp_mode(self, x, repeat_ratio, frame_num):
        ...
        output_dict = {
                'htsat_layer_output': x_downsample,
                'last_output': x,
    ...
    def forward(self, x: torch.Tensor, mixup_lambda = None, infer_mode = False, asp_mode = False):# out_feat_keys: List[str] = None):
        if self.config.input_ov_stft2melchannel:
            x = self.spec_extractor_stft_abs(x)
            x = self.ov_stft2mel(x, False)
        elif self.config.input_ov_stft2complex:
            (x_real, x_imag) = self.spec_extractor_stft(x)
            x = torch.cat((x_real, x_imag), dim=1)
            x = self.ov_stft2complex(x, False)
            # print("test", x.shape, x.max(), x.min())
        else:
        ...
        if infer_mode:
            ...
            # in asp mode, we need to return every layer of network
            frame_num = x.shape[2]
            target_T = int(self.spec_size * self.freq_ratio)
            repeat_ratio = math.floor(target_T / frame_num)
            x = x.repeat(repeats=(1,1,repeat_ratio,1))
            x = self.reshape_wav2img(x)
            output_dict = self.forward_features_asp_mode(x, repeat_ratio, frame_num)
